...
if [ -z "$CLOUDSQL_DIR" ]; then
    # Note: This directory WILL NOT be commited into the repo
    # If you are someone who has to deploy this resource, you MUST
    # ask for the required files
    CLOUDSQL_DIR=cloudsql
fi

if [ -z "$CLOUDSQL_SECRET_NAME" ]; then
    CLOUDSQL_SECRET_NAME=cloudsql
fi
